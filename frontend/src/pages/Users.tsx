...
import { Plus, Edit, Trash2, Shield, User as UserIcon, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Users: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditMode && selectedUserId) {
      ...
      updateUserMutation.mutate({ id: selectedUserId, data: updatePayload });
    } else {
  ...
  const columnDefs: ColDef[] = [
    ...
    { headerName: 'Role ID', field: 'role_id', width: 100 },
    { 
      headerName: 'Status', 
      field: 'is_active', 
      width: 100,
  ...
  return (
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 rounded-2xl bg-[#fceae7] flex items-center justify-center shadow-sm">
            <UserIcon size={24} className="text-[#7f1d1d]" />
          </div>
          <div>
      ...
      <div className="flex-1 bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden relative min-h-[400px]">
        ...
        <div className="ag-theme-quartz h-full w-full users-grid">
          <AgGridReact
            theme="legacy"
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-slate-900">{isEditMode ? 'Edit User' : 'Add New User'}</h2>
                <button 
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Password {isEditMode && '(Leave blank to keep current)'}</label>
                  <input 
                    type="password" 
                    required={!isEditMode}
                    className="w-full mt-1 px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#c0392b] focus:border-transparent text-sm"
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Role ID</label>
                  <input 
                    type="number" 
                    className="w-full mt-1 px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#c0392b] focus:border-transparent text-sm"
                ...
                <div className="pt-2 flex justify-end gap-3">
                  ...
                  >
                    {createUserMutation.isPending || updateUserMutation.isPending ? 'Saving...' : (isEditMode ? 'Update User' : 'Create User')}
                  </button>
                </div>
              </form>
            </motion.div>
      ...
      <style>{`
        .users-grid .ag-header,
        .users-grid .ag-header-row,
        .users-grid .ag-header-cell {
          background-color: #fceae7 !important;
        }
        .users-grid .ag-header-cell-label {
